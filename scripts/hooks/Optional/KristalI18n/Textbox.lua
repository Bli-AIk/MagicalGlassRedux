...
-- The i18n implementation keeps this helper in its text module. Newer builds
-- may expose it on the library instance; older builds do not, so retain a
-- small Registry fallback below. The adapter therefore stays optional and
-- does not add a hard dependency to the core MGR library.
local function tryItemKeyResolver(resolver, owner, value, field)
    ...
    local ok, key, id
    if owner ~= nil then
        ok, key, id = pcall(resolver, owner, value, field)
        if ok and type(key) == "string" then
            return key, id
        end
    end

    ok, key, id = pcall(resolver, value, field)
    if ok and type(key) == "string" then
        return key, id
    end
    return nil
...
local function getItemKeyFromI18N(value, field)
    local library = Mod and Mod.libs and Mod.libs["kristalI18n"]
    if not library then
        return nil
    end

    local key, id = tryItemKeyResolver(
        library.getItemTextLocalizationKey, nil, value, field
    )
    if key then
        return key, id
    end

    -- Accommodate an API grouped under a public `text`/`modules.text` table.
    local text_module = library.text or (library.modules and library.modules.text)
    if text_module then
        key, id = tryItemKeyResolver(
            text_module.getItemTextLocalizationKey, text_module, value, field
        )
        if key then
            return key, id
        end
    ...
    local global_library = rawget(_G, "kristalI18n")
    if global_library and global_library ~= library then
        key, id = tryItemKeyResolver(
            global_library.getItemTextLocalizationKey, global_library, value, field
        )
        if key then
            return key, id
        end
...
local item_lookup = {}
local item_lookup_lower = {}
local item_lookup_registry = nil
local item_lookup_count = 0
local language_item_lookup = {}
local language_item_lookup_lower = {}
local language_lookup_signature = nil

local function addLookupValue(lookup, value, id)
    if type(value) ~= "string" or value == "" or type(id) ~= "string" then
        return
    end

    lookup[value] = lookup[value] or {}
    for _, existing in ipairs(lookup[value]) do
        if existing == id then
            return
        end
    end
    table.insert(lookup[value], id)
end

local function addItemLookup(value, id)
    if type(value) ~= "string" or value == "" then
        return
    end
    addLookupValue(item_lookup, value, id)
    addLookupValue(item_lookup_lower, value:lower(), id)
end
...
local function rebuildItemLookup()
    ...
    if item_lookup_registry == Registry.items and item_lookup_count == count then
        return
    end

    item_lookup = {}
    item_lookup_lower = {}
    item_lookup_registry = Registry.items
    item_lookup_count = count
    ...
    for registry_id, item_data in pairs(Registry.items) do
        if type(item_data) == "function" or type(item_data) == "table" then
            ...
            if ok and type(item) == "table" then
                ...
                addItemLookup(item.name, id)
                -- A few MGR items use a distinct use_name in raw text.
                addItemLookup(item.use_name, id)
            end
        end
    end
end
...
local function itemIdFromKey(key)
    ...
    -- Chapter-specific item names still carry the same item id. Keep this
    -- parser local so a public i18n resolver can return either base or chapter
    -- keys without making the adapter depend on its implementation details.
    return key:match("^item_(.-)_name$")
        or key:match("^item_(.-)_name_chapter_%d+$")
end

local function addLanguageItemLookup(key, value)
    local id = itemIdFromKey(key)
    if not id or type(value) ~= "string" or value == "" then
        return
    end
    addLookupValue(language_item_lookup, value, id)
    addLookupValue(language_item_lookup_lower, value:lower(), id)
end

local function rebuildLanguageItemLookup()
    local current = Game and Game.langStr
    local base = Game and Game.langBaseStr
    local signature = tostring(current) .. ":" .. tostring(base)
    if language_lookup_signature == signature then
        return
    end

    language_item_lookup = {}
    language_item_lookup_lower = {}
    language_lookup_signature = signature

    for _, language_table in ipairs({ current, base }) do
        if type(language_table) == "table" then
            for key, value in pairs(language_table) do
                addLanguageItemLookup(key, value)
            end
        end
    end
end

local function getItemCandidates(value)
    rebuildItemLookup()
    local direct = item_lookup[value]
        or (type(value) == "string" and item_lookup_lower[value:lower()])
    if direct then
        return direct
    end

    -- Registry is unavailable in a few early-loading and test contexts. The
    -- loaded language tables still provide an authoritative name -> id map.
    rebuildLanguageItemLookup()
    return language_item_lookup[value]
        or (type(value) == "string" and language_item_lookup_lower[value:lower()])
end

local function localizedItemNameById(id, fallback)
    if type(id) ~= "string" then
        return fallback, id
    end

    local name_key = "item_" .. id .. "_name"
    if hasLocalization(name_key) then
        return Game:loc(name_key), id
    end
    return fallback, id
end

local function resolveItemName(value)
    if type(value) ~= "string" or value == "" then
        return value, nil
    end

    local key, id = getItemKeyFromI18N(value, "name")
    if key then
        id = id or itemIdFromKey(key)
        local localized = localizedItemNameById(id)
        if localized ~= nil then
            return localized, id
        end
        if hasLocalization(key) then
            return Game:loc(key), id
        end
    end

    local candidates = getItemCandidates(value)
    if candidates then
        for _, candidate_id in ipairs(candidates) do
            local localized = localizedItemNameById(candidate_id)
            if localized ~= nil then
                return localized, candidate_id
            end
        end
    end
...
local function localizeItemName(value)
    return resolveItemName(value)
end

-- LightMenu and LightBattle are separate hooks loaded from the same optional
-- adapter directory. Expose the resolver through MGR so those draw-time paths
-- use exactly the same item-id/name lookup as this Textbox hook.
if HasI18N and Mod and Mod.libs and Mod.libs["magical-glass"] then
    Mod.libs["magical-glass"].i18n_localizeItemName = localizeItemName
end
